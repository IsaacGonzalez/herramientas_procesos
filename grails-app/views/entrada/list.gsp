			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="descripcion" title="${message(code: 'entrada.descripcion.label', default: 'Descripcion')}" />
					
						<g:sortableColumn property="nombre" title="${message(code: 'entrada.nombre.label', default: 'Nombre')}" />
				...
				<g:each in="${entradaInstanceList}" status="i" var="entradaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${entradaInstance.id}">${fieldValue(bean: entradaInstance, field: "descripcion")}</g:link></td>
					
						<td>${fieldValue(bean: entradaInstance, field: "nombre")}</td>
